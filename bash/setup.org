...
- [[#update-system][Update System]]
  - [[#determine-if-running-in-termux][Determine if running in Termux]]
  - [[#otherwise-skip-termux-package-updates][Otherwise skip Termux package updates]]
...
Note that there is no ~Quit~ option in this dialogue because replying ~No~ acts in the same way as ~Quit~ normally would.
#+begin_src bash :tangle setup.sh
  case $DISTRO_ID in
      unknown)
...
** Otherwise skip Termux package updates
#+begin_src bash :tangle setup.sh
  else
...
*** Prompt user to install optional flatpak packages
#+begin_src bash :tangle setup.sh
